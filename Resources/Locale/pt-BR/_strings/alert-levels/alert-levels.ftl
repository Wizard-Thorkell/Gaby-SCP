...
alert-level-blue-announcement = Há uma ameaça desconhecida na estação. A segurança está autorizada a realizar buscas aleatórias. Para agilizar os procedimentos, solicitamos aos colaboradores que verifiquem a presença de carteiras de identidade em seus PDAs.
...
alert-level-red-announcement = Há uma ameaça conhecida na estação. A segurança tem o direito de usar força letal quando necessário. Todos os tripulantes, com exceção dos oficiais, são obrigados a dirigir-se aos seus respectivos departamentos e aguardar novas instruções antes de cancelar o código.
alert-level-red-instructions = A tripulação é obrigada a obedecer às ordens legais dos oficiais de segurança. Mude o modo de operação do seu traje para o modo “Coordenadas” e encontre-se no seu departamento.
...
alert-level-delta-announcement = A estação está sob ameaça de destruição iminente. Os membros da tripulação são incentivados a ouvir os chefes de departamento para obter informações adicionais. A segurança foi ordenada a operar sob o protocolo Delta. Todos os airlocks receberam acesso de emergência.
